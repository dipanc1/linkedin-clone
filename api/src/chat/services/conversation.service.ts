...
export class ConversationService {
  ...
  createConversation(creator: User, friend: User): Observable<Conversation> {
    return this.getConversation(creator.id, friend.id).pipe(
      switchMap((conversation: Conversation) => {
        const doesConcversationExist = !!conversation;
        if (!doesConcversationExist) {
  ...
  getConversationForUser(userId: number): Observable<Conversation[]> {
    return from(
      this.conversationRepository
        .createQueryBuilder("conversation")
        .leftJoin("conversation.users", "user")
  ...
  joinConversation(
    ...
    return this.getConversation(userId, friendId).pipe(
      switchMap((conversation: Conversation) => {
        if (!conversation) {
          console.warn(
            `No Converstion exists for userId: ${userId} and friendId: ${friendId}`
          );
          return of();
        }
